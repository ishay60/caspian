...
def _auto_detect_key(chords: list[Chord]) -> Key:
    ...
    for chord in (first, last):
        ...
        candidates[(chord.root_name, mode_key)] += 3

    # Heuristic 2: Most common root (weight: 2)
    root_counts = Counter(c.root_name for c in chords)
    most_common_root, _ = root_counts.most_common(1)[0]
    # Check if most common appears as minor or major
    minor_count = sum(1 for c in chords if c.root_name == most_common_root
                      and c.quality in (ChordQuality.MINOR, ChordQuality.MINOR7))
    major_count = sum(1 for c in chords if c.root_name == most_common_root
    ...
    candidates[(most_common_root, mode_key)] += 2
    ...
    for i in range(len(chords) - 1):
        if chords[i].quality in (ChordQuality.MAJOR, ChordQuality.DOMINANT7):
            ...
            if chords[i + 1].root == expected_tonic:
                tonic_name = chords[i + 1].root_name
                mode = "natural_minor" if chords[i + 1].quality in (
                    ChordQuality.MINOR, ChordQuality.MINOR7
                ) else "major"
                candidates[(tonic_name, mode)] += 5
    ...
    for root_pitch in range(12):
        ...
        for mode_name in ("natural_minor", "major"):
            ...
            candidates[(rname, mode_name)] += coverage

    if not candidates:
        return Key.from_name("C", "major")

    best = candidates.most_common(1)[0][0]
    return Key.from_name(best[0], best[1])
